...
HTTP-расчёт синхронный; клиент ждёт /pack до 120 секунд, nginx — до 125 секунд. У каталоговых запросов таймаут 15 секунд. Отмена на клиенте не прерывает работу CPU; процессы Z3 ограничены своим общим сроком 1–60 секунд, резервная эвристика выполняется отдельно. Лимит до 8 solver-процессов общий для запросов одного процесса API; при масштабировании Uvicorn его нужно учитывать отдельно.

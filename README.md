...
числу коробок и пустому объёму. В форме задаются 1–60 секунд поиска и 1–8 процессов
(по умолчанию 10 секунд и 4 процесса). Для машины с 10 ядрами можно выбрать 8.
Интерфейс показывает, доказан ли оптимум или использован резервный план. Полная модель
Z3 ограничена 16 предметами и 64 полезными коробочными слотами; при превышении
применяется явно обозначенная эвристика с полной опорой. Подробнее: [Z3_ENGINE.md](docs/Z3_ENGINE.md).
...
В Linux/macOS: `PLAYWRIGHT_BASE_URL=http://127.0.0.1:8080 PLAYWRIGHT_EXTERNAL_SERVER=1 DUNCARBOX_LIVE_E2E=1 npm run test:e2e`. Одиннадцать live-тестов не подменяют API; без флага они явно пропускаются. Они создают и удаляют только свою тестовую запись каталога.

После pull и добавления stress-теста: **368 backend-тестов пройдены, 1 тяжёлый тест пропущен без явного флага; 85 frontend-тестов и production build успешны**. На этапе Z3 также пройдены 25 браузерных сценариев и весь Docker Compose. Отдельные нагрузочные прогоны и границы — в [AGENT_HANDOFF.md](docs/AGENT_HANDOFF.md).

## Сценарий показа и ограничения

Отдельный тяжёлый тест **10 000 предметов / 100 SKU / 8 типов коробок**:
из `backend/` выполните `uv run python -m app.packing.stress_benchmark --timeout-seconds 600`.
Данные, отчёт и успешный результат сохраняются в `.cache/stress-10000/`.
Тест работает напрямую с движком, выше лимита HTTP API, и не входит в обычный
быстрый pytest-прогон. Для режима Z3 добавьте `--algorithm z3` (на таком размере
это проверка его резервной эвристики). Подробнее: [STRESS_TEST.md](docs/STRESS_TEST.md).

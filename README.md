...
**MVP интегрирован и проверен в Docker.** Сервер рассчитывает упаковку через `DeterministicPackingEngine` (`candidate-packing-v1`), хранит каталог в PostgreSQL и формирует инструкции по фактическим размещениям. Интерфейс поддерживает ввод заказа, каталог коробок, 3D/слои, пошаговую укладку, альтернативы, диагностику и экспорт в JSON. Для презентации доступен отдельный автономный демо-режим.
...
числу коробок и пустому объёму. Поиск работает без ограничения времени, до завершения
или ручной отмены. В форме задаётся число процессов (по умолчанию 4).
...
При ошибке решателя доступен проверенный резерв с полной опорой. Старый параметр
`solver_timeout_ms` принимается для совместимости, но не ограничивает расчёт. Подробнее: [Z3_ENGINE.md](docs/Z3_ENGINE.md).

Презентация для выступления: [DunCarBox_Презентация.pptx](DunCarBox_Презентация.pptx).
...
2. Переключайте коробки, шаги, 3D и слои; сравните альтернативы, выгрузите JSON.
...
Эвристика не гарантирует глобальный оптимум. Она учитывает геометрию, вес, остатки и минимум 80% опоры под основанием, но не центр масс, хрупкость или нагрузки. Расчёт не резервирует коробки. Лимиты на число единиц, строк товаров и типов коробок сняты. Все расчёты браузера используют фоновый API с прогрессом этапа, временем и отменой серверных процессов; общего HTTP-ожидания в 120 секунд больше нет. Синхронный /pack сохранён для API-клиентов. Каталоговые запросы — 15 секунд. Эвристика запускает до 12 независимых стратегий параллельно (solver_workers); Z3 и его предварительная эвристика работают без ограничения времени. При одинаковом количестве и объёме упакованных товаров приоритет имеет максимальное общее заполнение. Временные результаты фоновых задач доступны до 15 минут после завершения (до трёх последних задач) и не переживают перезапуск API. Авторизация и публичный production-сервис находятся за рамками локального хакатонного MVP.

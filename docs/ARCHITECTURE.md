...
flowchart TD
    ...
    Engine --> Packing[DeterministicPackingEngine]
...
MVP рассчитан на локальную демонстрацию: порты Docker опубликованы на loopback, авторизация и публичное развёртывание находятся вне текущего этапа. Runtime не использует LLM или внешнюю библиотеку упаковки. Расчёт синхронный; клиент ждёт /pack до 120 секунд, nginx — до 125 секунд. У каталоговых запросов таймаут 15 секунд. Отмена запроса на клиенте не прерывает уже запущенную работу CPU.

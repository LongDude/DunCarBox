...
- Поворот на дне разрешён всегда: при `allow_rotation=false` допустимы `LWH` и `WLH`,
  высота сохраняется. Если true — разрешена смена основания, максимум 6 перестановок.
  При одинаковых размерах используется первая допустимая ориентация указанного порядка.
...
до завершения, ошибки или отмены; геометрический поиск имеет общий бюджет
50 000 000 единиц работы Z3 на процесс, включая все повторы. При его исчерпании
возвращается лучший проверенный план с `reason=resource_limit`, без доказательства
оптимальности. Отмена останавливает дочерние процессы.
...
| OptimizationInfo | status (`optimal/feasible/fallback`), reason (`completed/resource_limit/solver_error`; `time_limit/size_limit` для старых результатов), workers (реально запущенные процессы, >=0), time_limit_ms=null (старые положительные значения читаются), support_ratio=1.0 |
...
или равно null в старых fixtures. Коробки отсортированы по заполнению по убыванию;
номера экземпляров и ссылки инструкций согласованы с новым порядком.
...
товар не вошёл в выбранный план при оптимизации заполнения; сам код не является
доказательством невозможности размещения в другом плане.
...
| POST | /pack/jobs | PackingRequest → 202 `{id,status,error,elapsed_seconds,timeout_seconds,stage,progress}` |
...
Оба алгоритма ставят на первое место общее заполнение: точное отношение объёма
упакованных товаров к суммарному объёму коробок. Затем максимизируются число
упакованных единиц и их объём; затем минимизируется количество коробок.
Частичный план может быть лучше полного. Пустой план имеет заполнение 0.
Цены коробок в контракте нет. Прежние пределы 16 экземпляров / 64 слота сняты.
Полезных слотов не больше общего числа единиц заказа и суммарного остатка.
Поиск продолжается до доказательства оптимальности, исчерпания бюджета, ошибки или отмены. При ошибке

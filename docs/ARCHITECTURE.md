...
Все расчёты в браузере используют POST /pack/jobs, короткий опрос статуса, загрузку результата и DELETE для отмены. Синхронный /pack сохранён для API-клиентов. Общее время фонового расчёта и поиск Z3 не ограничены; устаревшее solver_timeout_ms принимается для совместимости, но игнорируется. На этапе поиска Z3 progress=null: доля доказательства неизвестна. Число solver-процессов определяется запрошенным значением и доступными CPU, без константы 8.

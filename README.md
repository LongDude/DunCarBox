...
В Linux/macOS: `PLAYWRIGHT_BASE_URL=http://127.0.0.1:8080 PLAYWRIGHT_EXTERNAL_SERVER=1 DUNCARBOX_LIVE_E2E=1 npm run test:e2e`. Шесть live-тестов не подменяют API; без флага они явно пропускаются. Они создают и удаляют только свою тестовую запись каталога.

Проверено: **322 backend-теста, 71 frontend-тест, 19 браузерных сценариев**, production build и весь Docker Compose. Подробности и границы — в [AGENT_HANDOFF.md](docs/AGENT_HANDOFF.md).
...
Эвристика не гарантирует глобальный оптимум. Она учитывает геометрию, вес, остатки и минимум 80% опоры под основанием, но не центр масс, хрупкость или нагрузки. Расчёт не резервирует коробки. API принимает до 1000 единиц и 100 типов коробок; крупные разнородные заказы могут считаться заметно дольше demo. Клиент ждёт расчёт до 120 секунд, запросы каталога — до 15 секунд. Отмена в браузере прекращает ожидание; уже начатый синхронный поиск на сервере завершается самостоятельно. Авторизация и публичный production-сервис находятся за рамками локального хакатонного MVP.
...
| `backend/app/packing/` | Детерминированный движок, геометрия, диагностика и независимый валидатор |
